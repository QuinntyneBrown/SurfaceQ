...
- `src/SurfaceQ.Core` — pure logic, no Node: `ProjectLocator`, `ManifestReader`, `SourceFileWalker`, `PublicApiRenderer`, the docs side (`WorkspaceLocator`, `ApiDoc` model records, `MarkdownRenderer`), the inventory side (`WorkspaceProjectLocator` for apps+libs, `InventoryWalker`, `Inventory` model records, `InventoryRenderer`), and the FICD side (`Ficd` model records, `FicdFrontmatter` parser, `FicdMetadataReader`, `FicdRenderer`, `FicdScaffolder`).
- `src/SurfaceQ.Cli` — `System.CommandLine` entry point (`Program.cs`), command handlers (`GenerateCommand`/`CheckCommand`/`DiffCommand`/`DocsCommand`/`InventoryCommand`/`FicdCommand`/`FicdInitCommand`), `OutputPipeline`/`DocumentationPipeline`/`InventoryPipeline`/`FicdPipeline` (orchestration), verbosity `Writers`, `NodeResolver`, `UnifiedDiff`. Assembly name is `surfaceq`.
