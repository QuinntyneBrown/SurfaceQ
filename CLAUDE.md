...
- `src/SurfaceQ.Cli` — `System.CommandLine` entry point (`Program.cs`), command handlers (`GenerateCommand`/`CheckCommand`/`DiffCommand`/`DocsCommand`/`InventoryCommand`/`FicdCommand`), `OutputPipeline`/`DocumentationPipeline`/`InventoryPipeline`/`FicdPipeline` (orchestration), verbosity `Writers`, `NodeResolver`, `UnifiedDiff`. Assembly name is `surfaceq`.
