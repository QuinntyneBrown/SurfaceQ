...
- `docs` — document every library in a workspace as Markdown, one `API.md` per library (exit `0` ok, `2` error). `--project` is the workspace root; `--output <path>` (default `API.md`) is relative to each library directory. By default it hides classes that implement an interface exported by the same library (implementation details reached via injection token); `--include-implementations` shows them. `--services` restricts the document to Angular **services** — exported `@Injectable` classes the sidecar's `document` method tags with `role: "Service"` (the same `classRole` classifier `inventory` uses) — and, unless `--output` is given, writes `SERVICE_API.md` instead of `API.md`; services mode supersedes implementation-hiding (a service is the implementation a token hides, so service classes stay even when they implement an exported interface). Filtering lives in `DocumentationPipeline` (`ExcludeImplementations`, `OnlyServices`, `ExcludeDeprecated`, chosen in `SelectDeclarations`). Declarations tagged with the `@deprecated` JSDoc tag are **excluded by default** (declaration-level only — a deprecated member inside a non-deprecated declaration is kept and still flagged); `--include-deprecated-types` restores them, and they then render with a `Deprecated` column (reason / `yes` / `no`), a callout under heading-rendered interfaces/classes/enums, and a `Deprecations` summary table at the top. The deprecated-exclusion and services filters are docs-only: the `providers`/`ficd` pipelines call `DocumentationPipeline.Build` with the include-everything defaults, so they still see every declaration.
